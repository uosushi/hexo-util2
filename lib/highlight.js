...
function highlightUtil(str, options = {}) {
  ...
  const before = useHljs ? `<pre class="code"><code class="${classNames}">` : '<pre class="code">';
  ...
  if (gutter) {
    result += `<pre class="gutter" aria-hidden="true">${numbers}</pre>`;
  }

  result += before + content + after + '</figure>';
  return result;
}
